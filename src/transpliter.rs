...
pub fn transpile_line(line: &str) -> String {
	...
	// --- Separate comment (all operations via .get()) ---
	...
	if current.starts_with("customtype ") {
		return format!(
			"{}{}{}\n",
			" ".repeat(indent),
			handle_custom_type_decl(&current),
			comment
		);
	}
...
fn handle_pipeline(s: &str) -> String {
	...
	if let Some(eq_pos) = current.find('=') {
		...
		if !left.contains("->")
			&& !left.contains("+>")
			&& !current.get(eq_pos..).unwrap_or("").starts_with("==")
		{
...
fn handle_enumeration(s: &str, indent: usize) -> String {
	...
	let after_from = trimmed["enumeration from".len()..].trim_start();
	let to_pos = match after_from.find(" to ") {
		Some(p) => p,
		None => return s.to_string(),
	};
	let expr = after_from[..to_pos].trim();
	let after_to = after_from[to_pos + 4..].trim_start();
	let brace_pos = match after_to.find('{') {
		Some(p) => p,
		None => return s.to_string(),
	};
