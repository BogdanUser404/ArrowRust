...
// ANSI коды для оформления в терминале
...
pub fn process_dialect(input: String, lib_root: &str) -> String {
	...
	let mut strict_tabs = false; // Режим инквизиции табов

	for (line_idx, line) in input.lines().enumerate() {
		let line_num = line_idx + 1;
		let trimmed = line.trim_start();

		// 1. Обработка директив управления
		if trimmed.starts_with("//#") {
			...
			// 2. Проверка на пробелы в режиме NO_SPACE
			if strict_tabs && !line.is_empty() {
				// Если в начале строки пробел — это смертный грех
				if line.starts_with(' ') {
					// Сначала позорим жёлтым в stderr
					eprintln!(
						"{}[Style Error] Line {}: Detected spaces instead of tabs!{}",
						YELLOW, line_num, RESET
					);

					// А затем роняем всё к чертям
					panic!(
						"\n{}STYLE PANIC:{} \n\t\
			...
			// 3. Обработка контента (макросы или сырой код)
			if enabled {
